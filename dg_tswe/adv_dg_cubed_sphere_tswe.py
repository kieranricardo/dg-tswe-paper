import numpy as np
from dg_tswe.utils import gll, lagrange1st, cross_product, norm_L2
import meshzoo
import torch
from matplotlib import pyplot as plt
from dg_tswe.geometry import EquiangularFace, SadournyFace
import os


class AdvDGCubedSphereTSWE:
    def __init__(
            self, poly_order, nx, ny, g, f, eps, radius=1.0, device='cpu',
            solution=None, a=0.0, upwind=False, dtype=np.float32, damping=None,
            tau_func=lambda t, dt: t, tau=0, **kwargs):

        self.face_names = ['zp', 'zn', 'xp', 'xn', 'yp', 'yn']
        self.faces = {
            name: AdvDGCubedSphereFaceTSWE(
                name, poly_order, nx, ny, g, f, radius, eps, device, a=a, dtype=dtype, damping=None, bc='', tau=tau, upwind=upwind
            )
    ...
    def boundaries(self, sol=None):
        ...
        if sol is None:
            sol = {n: (self.faces[n].u, self.faces[n].v, self.faces[n].w, self.faces[n].h, self.faces[n].hb) for n in self.face_names}
    ...
    def time_step(self, dt=None, order=3, forcing=None):
        ...
        self.entropy_list.append(self.integrate(self.entropy()))
        ...
        u = {n: (self.faces[n].u, self.faces[n].v, self.faces[n].w, self.faces[n].h, self.faces[n].hb) for n in self.face_names}
        if order == 3:

            self.boundaries(u)
            k_1 = {n: self.faces[n].solve(*u[n], self.time, dt) for n in self.face_names}

            u_1 = {n: tuple(u[n][i] + dt * k_1[n][i] for i in range(5)) for n in self.face_names}
            self.boundaries(u_1)
            k_2 = {n: self.faces[n].solve(*u_1[n], self.time, dt) for n in self.face_names}

            u_2 = {n: tuple(0.75 * u[n][i] + 0.25 * (u_1[n][i] + dt * k_2[n][i]) for i in range(5)) for n in self.face_names}
            self.boundaries(u_2)
            ...
            u_1 = {n: tuple(u[n][i] + 0.5 * dt * k_1[n][i] for i in range(5)) for n in self.face_names}
            self.boundaries(u_1)
            k_2 = {n: self.faces[n].solve(*u_1[n], self.time, dt) for n in self.face_names}

            u_2 = {n: tuple(u_1[n][i] + 0.5 * dt * k_2[n][i] for i in range(5)) for n in self.face_names}
            self.boundaries(u_2)
            k_3 = {n: self.faces[n].solve(*u_2[n], self.time, dt) for n in self.face_names}

            u_3 = {n: tuple((2 / 3) * u[n][i] + (1 / 3) * u_2[n][i] + (1 / 6) * dt * k_3[n][i] for i in range(5)) for n in self.face_names}
            self.boundaries(u_3)
    ...
    def save_restart(self, fn_template, directory):
        vars = ['u', 'v', 'w', 'h', 'hb']
        state = {n: (self.faces[n].u, self.faces[n].v, self.faces[n].w, self.faces[n].h, self.faces[n].hb) for n in self.face_names}
        for name in self.face_names:
    ...
    def save_diagnostics(self, fn_template, directory):
        diagnostics = np.stack([self.time_list, self.energy_list, self.entropy_list, self.buoyancy_list, self.mass_list])
...
class AdvDGCubedSphereFaceTSWE:
    """
    ...
    def __init__(
            ...
            tau_func=lambda t, dt: t, bc='wall', tau=0.0, upwind=False, **kwargs):
    ...
    def boundaries(self, u, v, w, h, b, t):
    ...
    def get_dt(self):
        speed = self.wave_speed(self.u, self.v, self.w, self.h, self.b)
    ...
    def set_initial_condition(self, u, v, w, h, hb):
        ...
        self.b = torch.from_numpy((hb / h).astype(self.dtype)).to(self.device)
        ...
        self.b_left = torch.zeros((self.ny, self.nx + 1, self.n), dtype=self.tmp1.dtype).to(self.device)
        self.b_right = torch.zeros((self.ny, self.nx + 1, self.n), dtype=self.tmp1.dtype).to(self.device)
        self.b_up = torch.zeros((self.ny + 1, self.nx, self.n), dtype=self.tmp1.dtype).to(self.device)
        self.b_down = torch.zeros((self.ny + 1, self.nx, self.n), dtype=self.tmp1.dtype).to(self.device)
        ...
        self.boundaries(self.u, self.v, self.h, self.w, self.b, 0)

    def integrate(self, q):
        return (q * self.weights * abs(self.J)).sum()

    def entropy(self, u=None, v=None, w=None, h=None, b=None):
        if u is None:
            u = self.u
        if v is None:
            v = self.v
    ...
    def energy(self, u=None, v=None, w=None, h=None, b=None):
        if u is None:
            u = self.u
        if v is None:
            v = self.v
    ...
    def dEdt(self):
        u, v, w, h, b = self.u, self.v, self.w, self.h, self.b
        dudt, dvdt, dwdt, dhdt, dbdt = self.solve(u, v, w, h, b, 0, 0)
        # E = 0.5 * h * |u|^2 + 0.5 * b * h ^ 2
        dEdt = h * (u * dudt + v * dvdt + w * dwdt)
        dEdt += (0.5 * (u ** 2 + v ** 2 + w ** 2) + h * b) * dhdt
        dEdt += 0.5 * h ** 2 * dbdt
        return dEdt
    ...
    def wave_speed(self, u, v, w, h, hb):
        ...
        return torch.sqrt(u ** 2 + v ** 2 + w ** 2) + np.sqrt(hb)
    ...
    def solve(self, u, v, w, h, hb, t, dt, *, verbose=False):
        ...
        h_xflux, h_yflux = self.phys_to_contra(h_xflux, h_yflux, h_zflux)  # flux is in contravariant form
        ...
        # upper boundary
        c_up = self.wave_speed(self.u_up, self.v_up, self.w_up, self.h_up, self.hb_up)
        c_down = self.wave_speed(self.u_down, self.v_down, self.w_down, self.h_down, self.hb_down)
        c_ve = 0.5 * (c_up + c_down)
        c_right = self.wave_speed(self.u_right, self.v_right, self.w_right, self.h_right, self.hb_right)
        c_left = self.wave_speed(self.u_left, self.v_left, self.w_left, self.h_left, self.hb_left)
        ...
        # interior
        b_xflux, b_yflux, b_zflux = self.bflux(u, v, w, h, hb)
        b_xflux, b_yflux = self.phys_to_contra(b_xflux, b_yflux, b_zflux)  # flux is in contravariant form
        bdiv = torch.einsum('fgcd,abcd->fgab', b_xflux * self.J, self.ddxi)
        bdiv += torch.einsum('fgcd,abcd->fgab', b_yflux * self.J, self.ddeta)
        bdiv /= self.J
        verbose = False

        dbdxi = torch.einsum('fgcd,abcd->fgab', b, self.ddxi)
        dbdeta = torch.einsum('fgcd,abcd->fgab',b, self.ddeta)
        dhbdxi = torch.einsum('fgcd,abcd->fgab', hb, self.ddxi)
        dhbdeta = torch.einsum('fgcd,abcd->fgab', hb, self.ddeta)
        dhdxi = torch.einsum('fgcd,abcd->fgab', h, self.ddxi)
        dhdeta = torch.einsum('fgcd,abcd->fgab', h, self.ddeta)

        out = -self.weights * self.J * 0.5 * (bdiv + b * div + dbdxi * h_xflux + dbdeta * h_yflux)

        # boundary
        b_up_flux_x, b_up_flux_y, b_up_flux_z = self.bflux(self.u_up, self.v_up, self.w_up, self.h_up, self.hb_up)
        b_down_flux_x, b_down_flux_y, b_down_flux_z = self.bflux(self.u_down, self.v_down, self.w_down, self.h_down, self.hb_down)
        b_right_flux_x, b_right_flux_y, b_right_flux_z = self.bflux(self.u_right, self.v_right, self.w_right, self.h_right, self.hb_right)
        b_left_flux_x, b_left_flux_y, b_left_flux_z = self.bflux(self.u_left, self.v_left, self.w_left, self.h_left, self.hb_left)

        # fluxes through boundary
        b_up_flux = b_up_flux_y * self.eta_y_up + b_up_flux_x * self.eta_x_up + b_up_flux_z * self.eta_z_up
        b_down_flux = b_down_flux_y * self.eta_y_down + b_down_flux_x * self.eta_x_down + b_down_flux_z * self.eta_z_down
        b_right_flux = b_right_flux_y * self.xi_y_right + b_right_flux_x * self.xi_x_right + b_right_flux_z * self.xi_z_right
        b_left_flux = b_left_flux_y * self.xi_y_left + b_left_flux_x * self.xi_x_left + b_left_flux_z * self.xi_z_left

        if self.upwind:
            mask = h_flux_vert >= 0
            b_hat_ve = b_down * mask + b_up * (~mask)
            mask = h_flux_horz >= 0
            b_hat_ho = b_left * mask + b_right * (~mask)
        else:
            b_hat_ve = b_ve
            b_hat_ho = b_ho

        b_flux_vert = b_hat_ve * h_flux_vert
        b_flux_horz = b_hat_ho * h_flux_horz

        self.tmp1[:, :, -1] = (b_flux_vert[1:] - b_down_flux[1:]) * (self.weights_x * self.J_vertface[:, :, -1])
        self.tmp1[:, :, 0] = -(b_flux_vert[:-1] - b_up_flux[:-1]) * (self.weights_x * self.J_vertface[:, :, 0])
        self.tmp2[:, :, :, -1] = (b_flux_horz[:, 1:] - b_left_flux[:, 1:]) * (self.weights_x * self.J_horzface[..., -1])
        self.tmp2[:, :, :, 0] = -(b_flux_horz[:, :-1] - b_right_flux[:, :-1]) * (self.weights_x * self.J_horzface[..., 0])
        out -= (self.tmp1 + self.tmp2)

        b_k = out / (self.J * self.weights)
        ...
        out -= 0.5 * self.J * self.weights * 0.5 * (b * dhdxi + dhbdxi - h * dbdxi)

        # self.tmp1[:, :, -1] += 0.5 * (b_hat_ve * (self.h_up - self.h_down))[1:] * self.weights_x * (self.J_vertface / self.J_eta)[:, :, -1]
        # self.tmp1[:, :, 0] += - 0.5 * (b_hat_ve * (self.h_down - self.h_up))[:-1] * self.weights_x * (self.J_vertface / self.J_eta)[:, :, 0]
        self.tmp2[:, :, :, -1] += 0.25 * (b_hat_ho * (self.h_right - self.h_left))[:, 1:] * self.weights_x * (self.J_horzface / self.J_xi)[..., -1]
        self.tmp2[:, :, :, 0] += - 0.25 * (b_hat_ho * (self.h_left - self.h_right))[:, :-1] * self.weights_x * (self.J_horzface / self.J_xi)[..., 0]
        ...
        out -= 0.5 * self.J * self.weights * 0.5 * (b * dhdeta + dhbdeta - h * dbdeta)

        self.tmp1[:, :, -1] += 0.25 * (b_hat_ve * (self.h_up - self.h_down))[1:] * self.weights_x * (self.J_vertface / self.J_eta)[:, :, -1]
        self.tmp1[:, :, 0] += - 0.25 * (b_hat_ve * (self.h_down - self.h_up))[:-1] * self.weights_x * (self.J_vertface / self.J_eta)[:, :, 0]
        # self.tmp2[:, :, :, -1] += 0.5 * (b_hat_ho * (self.h_right - self.h_left))[:, 1:] * self.weights_x * (self.J_horzface / self.J_xi)[..., -1]
        # self.tmp2[:, :, :, 0] += - 0.5 * (b_hat_ho * (self.h_left - self.h_right))[:, :-1] * self.weights_x * (self.J_horzface / self.J_xi)[..., 0]
        ...
        return u_k, v_k, w_k, h_k, b_k

    def riemann_solve(self, u, v, w, h):
        # normal velocities
    ...
    def b_H1(self):
        ...
        b_sum = torch.zeros_like(b) + b * self.J * self.weights
        ...
        b_sum[0, :, 0] = b_sum[0, :, 0] + b_down[0] * Jw[0, :, 0]
        b_sum[-1, :, -1] = b_sum[-1, :, -1] + b_up[-1] * Jw[-1, :, -1]
        b_sum[:, 0, :, 0] = b_sum[:, 0, :, 0] + b_left[:, 0] * Jw[:, 0, :, 0]
        b_sum[:, -1, :, -1] = b_sum[:, -1, :, -1] + b_right[:, -1] * Jw[:, -1, :, -1]

        for tnsr in [b_sum, h_sum]:
