...
class DGCubedSphereFaceTSWE:
    ...
    def set_initial_condition(self, u, v, w, h, hb):
        ...
        self.boundaries(self.u, self.v, self.h, self.w, self.hb, 0)

    def integrate(self, q):
        return (q * self.weights * abs(self.J)).sum()

    def entropy(self, u=None, v=None, w=None, h=None, hb=None):
        if u is None:
            u = self.u
        if v is None:
            v = self.v
    ...
    def dEdt(self):
        ...
        dudt, dvdt, dwdt, dhdt, dhbdt = self.solve(u, v, w, h, hb, 0, 0)
        # E = 0.5 * h * |u|^2 + 0.5 * bh * h
