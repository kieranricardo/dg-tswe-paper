...
class CubedSphereSWE:
    def __init__(
            self, poly_order, nx, ny, g, f, eps, radius=1.0, device='cpu',
            solution=None, a=0.0, dtype=np.float32, damping=None,
            tau_func=lambda t, dt: t, **kwargs):

        self.face_names = ['zp', 'zn', 'xp', 'xn', 'yp', 'yn']
        self.faces = {
            name: CubedSphereFace(
                name, poly_order, nx, ny, g, f, radius, eps, device, a=a, dtype=dtype, damping=None, bc=''
            )
    ...
    def q_damping(self, sol, k, dt):
        ...
        if self.damping is not None:
            ...
            for name in self.face_names:
                face = self.faces[name]
                u, v, w, h = sol[name]
                u_k, v_k, w_k, h_k = k[name]
                q = face.q(u, v, w, h)

                dqdxi = torch.einsum('fgcd,abcd->fgab', q, face.ddxi)
                dqdeta = torch.einsum('fgcd,abcd->fgab', q, face.ddeta)

                dqdx, dqdy, dqdz = face.cov_to_phys(dqdxi, dqdeta, 0)
                kdqdx, kdqdy, kdqdz = cross_product([face.kx, face.ky, face.kz], [dqdx, dqdy, dqdz])

                q2_ht_1 = face.integrate(q * q * h_k)
                q_qht_1 = -face.integrate(kdqdx * u_k + kdqdy * v_k + kdqdz * w_k)
                chain_rule_error = 2 * q_qht_1 - q2_ht_1

                udivq = u * dqdx + v * dqdy + w * dqdz
                tmp = udivq * h

                denominator += face.integrate(tmp * udivq)
                numerator += 0.5 * chain_rule_error

            tau = numerator / denominator
            # print('tau:', tau, denominator)

            tau = self.tau_func(tau, dt)
            ...
            for name in self.face_names:
                ...
                u_k, v_k, w_k, h_k = k[name]
                q = face.q(u, v, w, h)
                cor = cross_product([face.kx, face.ky, face.kz], [u * h, v * h, w * h])
            ...
            if self.flag and (self.time > 0.05):
                enst_growth = 0
                self.flag = False
                ...
                for name in self.face_names:
                    ...
                    q2_ht_1 = face.integrate(q * q * h_k)
                    q_qht_1 = -face.integrate(kdqdx * u_k + kdqdy * v_k + kdqdz * w_k)
                    enst_growth += 2 * q_qht_1 - q2_ht_1

                print('Enstrophy growth:', enst_growth)
    ...
    def triangular_plot(self, ax, vmin=None, vmax=None, plot_func=None, cmap='nipy_spectral', latlong=False):
        ...
        vort = [(face.vorticity() - face.f).ravel() for face in self.faces.values()]
        if not latlong:
        ...
        if latlong:
            ...
            # ax.tricontour(
            #     x_coords[mask], y_coords[mask], vort[mask], colors='black',
            #     levels=np.linspace(-1.5e-4, 1.5e-4, 16), negative_linestyles='dashed', linewidths=0.5
            # )
...
class CubedSphereFace:
    ...
    def __init__(
            ...
            solution=None, a=0.0, dtype=np.float32, damping=None,
            tau_func=lambda t, dt: t, bc='wall', **kwargs):
    ...
    def solve(self, u, v, w, h, t, dt, *, verbose=False):
        ...
        h_right_flux_x, h_right_flux_y, h_right_flux_z = self.hflux(self.u_right, self.v_right, self.w_right,
                                                                    self.h_right)
        ...
        out = torch.einsum('fgcd,fgabcd->fgab', uv_flux, self.weak_grad_xi)  #
        self.tmp1[:, :, -1] = 0
        self.tmp1[:, :, 0] = 0
        self.tmp2[:, :, :, -1] = (uv_flux_horz - 0 * uv_left_flux)[:, 1:] * self.weights_x * (self.J_horzface / self.J_xi)[..., -1]
        self.tmp2[:, :, :, 0] = -(uv_flux_horz - 0 * uv_right_flux)[:, :-1] * self.weights_x * \
                                (self.J_horzface / self.J_xi)[..., 0]
        ...
        out = torch.einsum('fgcd,fgabcd->fgab', uv_flux, self.weak_grad_eta)

        self.tmp1[:, :, -1] = (uv_flux_vert - 0 * uv_down_flux)[1:] * self.weights_x * (self.J_vertface / self.J_eta)[:, :, -1]
        self.tmp1[:, :, 0] = -(uv_flux_vert - 0 * uv_up_flux)[:-1] * self.weights_x * (self.J_vertface / self.J_eta)[:, :, 0]
        self.tmp2[:, :, :, -1] = 0
        self.tmp2[:, :, :, 0] = 0
        ...
        cor = cross_product([self.kx, self.ky, self.kz], [u * h, v * h, w * h])
        q = self.q(u, v, w, h)
        ...
        # damping
        if self.damping is not None:
            dqdxi = torch.einsum('fgcd,abcd->fgab', q, self.ddxi)
            dqdeta = torch.einsum('fgcd,abcd->fgab', q, self.ddeta)

            dqdx, dqdy, dqdz = self.cov_to_phys(dqdxi, dqdeta, 0)
            kdqdx, kdqdy, kdqdz = cross_product([self.kx, self.ky, self.kz], [dqdx, dqdy, dqdz])

            q2_ht_1 = self.integrate(q * q * h_k)
            q_qht_1 = -self.integrate(kdqdx * u_k + kdqdy * v_k + kdqdz * w_k)
            chain_rule_error = 2 * q_qht_1 - q2_ht_1

            udivq = u * dqdx + v * dqdy + w * dqdz
            tmp = udivq * h

            denom = self.integrate(tmp * udivq)

            tau = 0.5 * chain_rule_error / denom
            tau = self.tau_func(tau, dt)
